...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    string * colLabels = new string [M.columns()];
    
    M.getColLabels(colLabels);
    
    Matrix pagerank(colLabels, 1, colLabels, M.columns(), 1.0/M.columns());
    ...
    while (iter < max_iter && !stable){
        ...
        Matrix new_pagerank(1, M.columns());
        new_pagerank = pagerank * M;
        
        cout << new_pagerank << endl;
        
        for (int i = 0; i < new_pagerank.columns(); i++)
            
            valor += abs(new_pagerank.at(0, i) - pagerank.at(0, i));
                
        if (valor < threshold)  stable = true;
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    for (int i = 0; i < idx.size(); i++){
        ...
        ub = idx.upper_bound(idx.at(i).getKey());
        
        for (int j = lb; j < ub; j++)
            
            for (int k = j + 1; k < ub; k++){
                
                brand1 = getEvent(evs, idx, j).getBrand();
                brand2 = getEvent(evs, idx, k).getBrand();
...
//    string * events = new string [idx.size()];
//    
//    for (int i = 0; i < idx.size(); i++)
//        
//        for (int j = 0; j < i; j++)
//            
//            if (idx.at(j).getKey() != idx.at(i).getKey()) 
//                
//                events[i] = idx.at(i).getKey();
//        Index idx_aux();
//        
//        for (int j = lb; j < ub; j++)
//            
//            idx_aux.add(idx.at(j));
//            
//        idx_aux = rawFilterIndex(evs, idx_aux, "Type", "purchase"); 
//        
//        for (int k = 0; k < idx_aux.size(); k++)
//            
//            for (int h = k+1; h < idx_aux.size(); h++){
//                
//                m_adj.at(evs.at(idx_aux.at(k).getPos()), evs.at(idx_aux.at(h).getPos()))++;
//                m_adj.at(evs.at(idx_aux.at(h).getPos()), evs.at(idx_aux.at(k).getPos()))++;
//            }              
//    }     
